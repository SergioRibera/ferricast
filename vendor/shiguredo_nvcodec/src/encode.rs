...
pub struct Encoder {
    ...
    // -------------------------------------------------------------
    // Persistent per-encoder resources (ferricast patch).
    //
    // Upstream allocated GPU memory, registered the input resource
    // and created an output bitstream buffer on every `encode()`
    // call, then freed them all immediately after. Each of those
    // round-trips into the NVIDIA driver costs ~1-3 ms; combined
    // they were the dominant per-frame cost (~10-15 ms at 1080p),
    // capping us below 60 fps. We allocate them once here and
    // reuse them for the lifetime of the encoder. Drop tears them
    // down before destroying the encoder session.
    //
    // Reconfigure (`reconfigure_encoder`) reallocates `device_input`
    // and re-registers `registered_resource` if the new frame size
    // is larger than what's already cached.
    // -------------------------------------------------------------
    /// Pre-allocated device-side input buffer (sized to
    /// `device_input_capacity` bytes). `0` until first
    /// `ensure_persistent_resources` call.
    device_input: sys::CUdeviceptr,
    device_input_capacity: usize,
    /// NVENC's registered handle for `device_input`. Null until
    /// first `ensure_persistent_resources` call. Re-registered if
    /// `device_input` is reallocated.
    registered_resource: sys::NV_ENC_REGISTERED_PTR,
    /// Pre-created NVENC output bitstream buffer. Reused every
    /// encode; we lock/unlock per frame but never destroy/recreate.
...
impl Encoder {
    ...
    pub fn new(config: EncoderConfig) -> Result<Self, Error> {
        unsafe {
            ...
            // Allocate the persistent CUDA + NVENC resources right
            // here so the first `encode()` call doesn't hide a
            // first-frame stall behind the steady-state path.
            lib.with_context(ctx, || encoder.ensure_persistent_resources())?;
    ...
    fn reconfigure_inner(&mut self, params: ReconfigureParams) -> Result<(), Error> {
        unsafe {
            ...
            if params.width.is_some() || params.height.is_some() {
                ...
                // ferricast patch: invalidate the persistent
                // resources so `ensure_persistent_resources` (called
                // from `encode_inner`) reallocates them at the new
                // size on the next encode.
                self.release_persistent_resources();
            }
            if let Some(fps_den) = params.framerate_den {
    ...
    fn encode_inner(&mut self, frame_data: &[u8], options: &EncodeOptions) -> Result<(), Error> {
        // ferricast patch: persistent device buffer + registered
        // resource + bitstream buffer were created in
        // `ensure_persistent_resources` (called from `new` and re-run
        // by `reconfigure_encoder` if frame size grew). We only need
        // to upload the new pixels, map the registered resource,
        // encode, lock+copy the bitstream, then unmap.
        self.ensure_persistent_resources()?;
        let device_input = self.device_input;
    ...
    /// Allocate (or grow) the persistent device input buffer,
    /// register it with NVENC, and create the persistent output
    /// bitstream buffer if any of them are missing or undersized.
    /// Idempotent: a no-op once everything is in place.
    fn ensure_persistent_resources(&mut self) -> Result<(), Error> {
        let needed = self.expected_frame_size;
        let need_input_realloc = self.device_input == 0 || self.device_input_capacity < needed;

        if need_input_realloc {
            // Free old input + unregister old resource handle
            // before swapping in new ones.
            if self.registered_resource != ptr::null_mut() {
                if let Some(unreg) = self.encoder.nvEncUnregisterResource {
                    unsafe {
                        let _ = unreg(self.h_encoder, self.registered_resource);
                    }
                }
                self.registered_resource = ptr::null_mut();
            }
            if self.device_input != 0 {
                let _ = self.lib.cu_mem_free(self.device_input);
                self.device_input = 0;
                self.device_input_capacity = 0;
            }

            let mut device_input: sys::CUdeviceptr = 0;
            self.lib.cu_mem_alloc(&mut device_input, needed)?;
            self.device_input = device_input;
            self.device_input_capacity = needed;
        }
        ...
        if self.registered_resource == ptr::null_mut() {
            unsafe {
                ...
                register_resource.resourceToRegister = self.device_input as *mut c_void;
        ...
        if self.bitstream_buffer == ptr::null_mut() {
            unsafe {
                ...
                self.bitstream_buffer = create_bitstream.bitstreamBuffer;
            }
        }
    ...
    /// Tear down persistent resources. Called from `Drop` before
    /// `nvEncDestroyEncoder` so handles are freed in the right
    /// order. Errors here are ignored — we're shutting down.
    fn release_persistent_resources(&mut self) {
        if self.bitstream_buffer != ptr::null_mut() {
            if let Some(destroy) = self.encoder.nvEncDestroyBitstreamBuffer {
                unsafe {
                    let _ = destroy(self.h_encoder, self.bitstream_buffer);
                }
            }
...
impl Drop for Encoder {
    fn drop(&mut self) {
        unsafe {
            // ferricast patch: free persistent resources first while
            // the CUDA context is still alive. We clone `lib` for
            // this so the closure doesn't try to borrow `self`
            // through both `self.lib` and the &mut method call.
            let lib_for_release = self.lib.clone();
            let _ = lib_for_release.with_context(self.ctx, || {
                self.release_persistent_resources();
                Ok(())
            });
